...
# Yangi egani BITTA usul bilan bering (ikkinchisini None qoldiring):
NEW_OWNER_USERNAME = None    # masalan: "TorinoUser"  (@ SIZ)
NEW_OWNER_TELEGRAM_ID = None # masalan: 123456789

CONFIRM = False              # True bo'lgandagina yoziladi; False — faqat ko'rsatadi
# ─────────────────────────────────────────────────────────────────────────

DB_PATH = os.getenv("DB_PATH", "efootball_bot.db")


def _fetch_owner(cursor, username, telegram_id):
    """Yangi egani username yoki telegram_id bo'yicha topadi."""
...
def main():
    ...
    try:
        ...
        # 2) Yangi ega
        owner = _fetch_owner(cursor, NEW_OWNER_USERNAME, NEW_OWNER_TELEGRAM_ID)
        if owner is None:
            print("\n❌ Yangi ega topilmadi. NEW_OWNER_USERNAME yoki "
                  "NEW_OWNER_TELEGRAM_ID ni to'g'ri to'ldiring.")
            return
        print("\n── YANGI ega (topildi) ──")
